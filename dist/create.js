...
const basePath = '../../../src';
const basePackagesPath = resolve(`${basePath}/packages`);
/* 驼峰转换-*/
...
function createFile(options) {
    ...
    */
    const indexTsTemplate = renderTemplate(resolve(`${basePath}/templates/${options.type}-index.ts.tmpl`), {
    ...
    fs.mkdirSync(`${basePackagesPath}/${folderName}`); // 创建 文件夹
    fs.mkdirSync(`${basePackagesPath}/${folderName}/demo`); //创建 子文件夹 demo
    fs.writeFileSync(`${basePackagesPath}/${folderName}/demo/README.md`, `# ${options.chineseName}  ${toLine(options.name)}`);
    fs.mkdirSync(`${basePackagesPath}/${folderName}/demo/demo1`); // demo 目录下创建 demo1 目录
    /**
     * 获取templates文件夹下需要的demo1-{type}-index.vue.tmpl模板  该模板应用于 demo1目录
     */
    const demo1IndexVue = renderTemplate(resolve(`${basePath}/templates/demo1-${options.type}-index.vue.tmpl`), {
        name: toCamel(options.name)
    });
    fs.writeFileSync(`${basePackagesPath}/${folderName}/demo/demo1/index.vue`, demo1IndexVue);
    fs.writeFileSync(`${basePackagesPath}/${folderName}/demo/demo1/index.md`, '基本用法');
    fs.mkdirSync(`${basePackagesPath}/${folderName}/src`);
    fs.writeFileSync(`${basePackagesPath}/${folderName}/index.ts`, indexTsTemplate);
    if (options.type === 'component') {
        /**
         * 获取templates文件夹下需要的src-index.vue.tmpl模板  该模板应用于 src目录 并且 type为 component
         */
        const srcIndexVue = renderTemplate(resolve(`${basePath}/templates/src-index.ts.tmpl`), {
            name: options.name,
            className: toLine(options.name),
            componentClassName: toCamel(options.name)
        });
        fs.writeFileSync(`${basePackagesPath}/${folderName}/src/index.ts`, srcIndexVue);
    }
    if (options.type === 'directive') {
        /**
         * 获取templates文件夹下需要的src-directive-index.ts.tmpl模板  该模板应用于 src目录 并且 type为 directive
         */
        const srcIndexDirective = renderTemplate(resolve(`${basePath}/templates/src-directive-index.ts.tmpl`), {});
        fs.writeFileSync(`${basePackagesPath}/${folderName}/src/index.ts`, srcIndexDirective);
    }
    if (options.type === 'tool') {
        const srcIndexTool = `export default {};
`;
        fs.writeFileSync(`${basePackagesPath}/${folderName}/src/index.ts`, srcIndexTool);
    }
    /**
     * 改写 map.json文件
     */
    const mapJson = require(`${basePath}/packages/map.json`);
    ...
    let mapJsonFormat = js_beautify_1.js_beautify(JSON.stringify(objKeySort(obj)), {
        indent_size: 2
    });
    fs.writeFileSync(`${basePackagesPath}/map.json`, mapJsonFormat);
    require('./router');
}
