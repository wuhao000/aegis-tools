                    ...
                    if (config.typeParameterReflects) {
                        ...
                        if (reflect) {
                            ...
                            const properties = typeParameters.map((it, index) => {
                                inter.name.typeParameters[index] = new ref_1.RefObject(typeChars[index]);
    ...
    paths.forEach(path => {
        ...
        requestTypes.forEach(method => {
            if (['GET', 'PUT', 'POST', 'DELETE'].includes(method)) {
                ...
                api.responseType = type_1.resolveResponseType(typeDefinition.responses['200']);
