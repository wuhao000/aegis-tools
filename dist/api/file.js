...
${data.interfaces.map(i => i.toString()).join('\n\n')}`, () => {
    ...
    fs.writeFile(`${data.config.apiRoot}/api-definition.ts`, tmpl_1.render(data.config.templates && data.config.templates.definition || 'node_modules/aegis-ui-tools/src/tmpl/definition.ts.tmpl', {
        content: str
    }), () => {
    });
