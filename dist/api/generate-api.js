...
function generateData(config, datas) {
    ...
    const apiImportList = [];
    apiImportList.push(new import_declaration_1.default(null, beanInterfaces.map(it => it.name.name), `./${file_1.beanDefFileName}`));
    return {
        config,
                                ...
                                if (level === 0) {
                                    ...
                                    if (existsInterface && existsInterface.keyPath) {
                                        if (existsInterface.keyPath.join('/') !== obj.definitionPath.join('/')) {
                                            name = exports.generateName(name, obj.definitionPath, interfaces.map(it => it.name));
                                        }
                                    }
                                    interfaces.push({
                                        name,
...
exports.generateName = (name, keyPath, allNames) => {
    if (!allNames.includes(name)) {
        ...
        while (allNames.includes(name) && paths.length) {
            name = normalizeName(pascal_1.default(paths.pop())) + exports.removePrefixDash(name);
        }
        return name;
    }
};
exports.removePrefixDash = (name) => {
    if (name.indexOf('_') === 0) {
        return exports.removePrefixDash(name.substr(1));
    }
    return name;
};
